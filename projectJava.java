...
public class projectJava extends mohid {
...
public static void rider_reader(String filename) throws IOException {
    ...
    try {
        ...
        int index_no = 0;
        File myFile = new File(filename + ".txt");
        int length = mohid.get_lines(filename + ".txt");
        ...
        if (myFile.length() == 0) {
            ...
            for (int row = 8; row < rana.ipAddress.length; row++) {
                for (int col = 0; col < rana.ipAddress[row].length; col++) {
                    if (rana.ipAddress[row][2].equals(home_address)) {
                        home_location += (rana.ipAddress[row][0] + " " + rana.ipAddress[row][1]);
                        break; // Mehdi ke hisab se error
                    }
                }

            }

            System.out.println("Your order is being delivered at: " + home_location);
...
// 2d array file maker
// public static void Receipt_file(String[][] raw2, String filename,String
// restaraunt_ip,String username_ip, String username) throws IOException {
// String[][] raw = raw2;
// // printMatrix(raw, matrixLength(raw));
...
// write.close();
// }
public static void Receipt_file(String[][] raw2, String filename, String restaraunt_ip, String username_ip,
...
public static String[][] generatingReceipt(String[][] raw, String[][] rawRes) {
    ...
    do {
        try {
            ...
            if (muji.isValid(number)) {
                System.out.println(number + " is valid!");
                orderID = muji.orderID();
            ...
            if (user == 1) {
                ...
                if (itemsLength > availableSpace) {
                    ...
                    if (check == soupRestaurant) {
                        printMatrix(mohid.soupRestaurant, matrixLength(mohid.soupRestaurant));
                    } else if (check == desiRestaurant) {
                        printMatrix(mohid.desiRestaurant, matrixLength(mohid.desiRestaurant));
                    } else if (check == pizzaRestaurant) {
                        printMatrix(mohid.pizzaRestaurant, matrixLength(mohid.pizzaRestaurant));
                    } else if (check == randomRestaurant4) {
                        printMatrix(mohid.randomRestaurant4, matrixLength(mohid.randomRestaurant4));
                    } else if (check == randomRestaurant5) {
                        printMatrix(mohid.randomRestaurant5, matrixLength(mohid.randomRestaurant5));
                    } else if (check == randomRestaurant6) {
                        printMatrix(mohid.randomRestaurant6, matrixLength(mohid.randomRestaurant6));
                    } else if (check == randomRestaurant7) {
                        printMatrix(mohid.randomRestaurant7, matrixLength(mohid.randomRestaurant7));
                    }
                    System.out.println();
